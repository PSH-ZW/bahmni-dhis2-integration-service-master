...
FROM (SELECT pi.*
      FROM %s pi
             INNER JOIN marker m on pi.date_created :: DATE BETWEEN m.last_synced_date AND '%s'
                                      AND category = 'instance' AND program_name = '%s') AS instanceTable
 FULL OUTER JOIN (

        (SELECT prog.* FROM %s prog
            INNER JOIN marker m ON prog.date_created :: DATE BETWEEN '%s' AND '%s'
 ...
 FULL OUTER JOIN (SELECT event.*
                   FROM %s event
                          INNER JOIN marker m
                            ON event.event_date :: DATE BETWEEN '%s' AND '%s'
