...
async function get(date, service, user, getAbove, logId, dispatch, onLoad = false) {
    let ajax = Ajax.instance();
    try {
        let response = await ajax.get("/dhis-integration/api/logs", {date, service, user, getAbove, logId, onLoad});
...
export function getLogs(date, service = '', user = '') {
    return async dispatch => {
        ...
        let logId = 0;
        await get(date, service, user, false, logId, dispatch, true);
    }
}
...
export function getLogsOnFilter(date, service = '', user = '') {
    return async dispatch => {
        ...
        let logId = 0;
        try {
            let response = await ajax.get("/dhis-integration/api/logs", {date, service, user, getAbove: true, logId, onLoad: true});
            if (response.length > 0) {
                dispatch(logs(response));
                dispatch(noEventsToDisplay());
...
export function getPrevPageLogs(date, service = '', user = '') {
    return async (dispatch, getState) => {
        ...
        let ajax = Ajax.instance();
        try {
            let response = await ajax.get("/dhis-integration/api/logs", {date, service, user, getAbove:true, logId, onLoad: false});
            if(response.length > 0) {
