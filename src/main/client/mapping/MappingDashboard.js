...
import {
  getAllMappings, exportMapping, exportAllMappings, importMappings
...
class MappingDashboard extends Component {
  ...
  async exportMapping(mappingName) {
    ...
    const mappingDetails = await exportMapping(mappingName, this.props.dispatch);
  ...
  parseFile(file) {
    ...
    reader.onload = () => {
      try {
        const parsedInfo = JSON.parse(reader.result);
