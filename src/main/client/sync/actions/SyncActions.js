...
export function handlePreview(startDate,endDate,mappingName){
    return async dispatch => {
        if (startDate != null && endDate == null)
            ...
            window.open(`/dhis-integration/preview/${mappingName.replaceAll(" ", "")}/${startDate.getFullYear()}-${startDate.getMonth() + 1}-${startDate.getDate()}/${endDate.getFullYear()}-${endDate.getMonth() + 1}-${endDate.getDate()}/${mappingName}`);
    }
}
export function getAllMappingsSyncInfo() {
    return async dispatch => {
        try {
            ...
            let ajax = Ajax.instance();
            let response = await ajax.get('/dhis-integration/api/getMappingsSyncInfo');
